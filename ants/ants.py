...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        def nearestBeeIteration(place, dis = 0):
            if place.is_hive:
                return None
            if random_bee(place.bees) == None:
                dis += 1
                return nearestBeeIteration(place.entrance, dis)
            if random_bee(place.bees)!= None and self.lower_bound <= dis <= self.upper_bound:
                return random_bee(place.bees)
            else:
                dis += 1
                return nearestBeeIteration(place.entrance, dis)
        return nearestBeeIteration(self.place)
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        # while pos:
        #     if pos.ant != None:
        #         if pos.ant.name == 'Fire':
        #             pos.ant.doubled = True
        #         else:
        #             pos.ant.double()
        #             if pos.ant.is_container and pos.ant.ant_contained is not None:
        #                 if pos.ant.ant_contained.name == 'Fire':
        #                     pos.ant.ant_contained.doubled = True
        #                 else:
        #                     pos.ant.ant_contained.double()
        #     pos = pos.exit
        while pos:
            if pos.ant is not None:
                if not pos.ant.doubled:
                    pos.ant.double()
                if pos.ant.is_container and pos.ant.ant_contained is not None:
                    # the pos.ant.ant_contained may change
                    if not pos.ant.ant_contained.doubled:
                        pos.ant.ant_contained.double()
