def duplicate_link(link, val):
    ...
    def link_iterator(link):
        ...
        if link.first == val:
            ...
            link_iterator(link.rest.rest)
        else:
            link_iterator(link.rest)
    return link_iterator(link)
...
def convert_link(link):
    ...
    #     else:
    #         convert_list.append(link.first)
    #         link = link.rest
    # return convert_list

    # Recursive solution
    def link_iterator(link, lst=[]):
        if link is Link.empty:
            return lst
        if type(link.first) == Link:
            return link_iterator(link.first.first)
        else:
            lst.append(link.first)
            return link_iterator(link.rest)
    return link_iterator(link)
