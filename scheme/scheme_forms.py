...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        functionName = signature.first
        expr = Pair(signature.rest, expressions.rest)
        env.bindings[functionName] = do_lambda_form(expr, env)
        return functionName
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    while expressions.rest != nil:
        term = scheme_eval(expressions.first, env)
        if is_scheme_false(term):
            return False
