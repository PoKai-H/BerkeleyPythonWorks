...
class VendingMachine:
    ...
    """
    "*** YOUR CODE HERE ***"
    def __init__(self, name, price, balance = 0, quantity = 0):
        self.name = name
        self.price = price
    ...
    def add_funds(self, bal):
        if self.quantity > 0:
            self.balance += bal
            return f'Current balance: ${self.balance}'
        else:
            change = self.balance + bal
            self.balance = 0
...
def store_digits(n):
    ...
    def store_digit_helper(digit_list):
        if len(digit_list) > 1:
            num = digit_list.pop(-1)
            return Link(num, store_digit_helper(digit_list))
        else:
            return Link(digit_list[-1])
    digit_list = []
    while n > 0:
        digit_list.append(n%10)
        n//=10
    return store_digit_helper(digit_list)
