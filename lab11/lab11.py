...
def floor_div(args):
    ...
    20
    """
    operantsA = args.first
    if isinstance(args.rest.first, int): 
        operantsB = 1
        while args.rest != nil:
            operantsB *= args.rest.first
            args = args.rest
    else:
        operantsB = calc_eval(args.rest.first)

    return operantsA // operantsB
...
def eval_and(expressions):
    ...
    """
    # BEGIN SOLUTION Q3
    # if isinstance(expressions.first, int) and expressions.rest == nil:
    #     return expressions.first
    # elif isinstance(expressions.first, bool) and expressions.rest.rest == nil:
    #     return expressions.first and expressions.rest.first
    # elif isinstance(expressions.first, int) and isinstance(expressions.rest.first, Pair):
    #     operantsB = calc_eval(expressions.rest.first)
    #     return expressions.first and operantsB
    # elif isinstance(expressions.first, Pair) and isinstance(expressions.rest.first, int):
    #     operantsA = calc_eval(expressions.first)
    #     return operantsA and expressions.rest.first
    # elif expressions == nil:
    #     return True
    # elif isinstance(expressions.first, Pair) and isinstance(expressions.rest.first, Pair):
    #     operantsA = calc_eval(expressions.first)
    #     operantsB = calc_eval(expressions.rest.first)
    #     return operantsA and operantsB
    # elif isinstance(expressions.first, int) and isinstance(expressions.rest.first, int):
    #     return expressions.first and expressions.rest.first 
    if expressions != nil:
        result = expressions.first
        while expressions.rest != nil:
            if result == False and isinstance(result, bool):
                return False
            if isinstance(expressions.rest.first, Pair):
                result = calc_eval(expressions.rest.first)
            else:
...
def eval_define(expressions):
    ...
    if isinstance(value, str) and value in OPERATORS:
        bindings[key] = OPERATORS[value]
    elif value in bindings:
        bindings[key] = bindings[value]
    else:
        bindings[key] = value
    return key
