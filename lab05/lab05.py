...
def replace_loki_at_leaf(t, lokis_replacement):
    ...
    new_branch = []
    for branch in branches(t):
        new_branch.append(replace_loki_at_leaf(branch, lokis_replacement))
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    x1 = get_lat(city_a) - lat
    y1 = get_lon(city_a) - lon
    x2 = get_lat(city_b) - lat
    y2 = get_lon(city_b) - lon
    dis_1_square = x1**2 + y1**2
    dis_2_square = x2**2 + y2**2
    return get_name(city_a) if dis_1_square < dis_2_square else get_name(city_b)
